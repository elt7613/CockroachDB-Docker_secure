...
# CockroachDB Secure Production Deployment Script - VM3 (Secondary Node)
# This script deploys a secondary CockroachDB node locally on VM3
...
check_prerequisites() {
    print_status "Checking prerequisites for VM3..."
    ...
    # Check if vm3-docker-compose.yml exists
    if [ ! -f "vm3-docker-compose.yml" ]; then
        print_error "vm3-docker-compose.yml not found in current directory"
        exit 1
    fi
...
display_config() {
    echo ""
    echo "=== VM3 Deployment Configuration ==="
    echo "VM1 IP: ${VM1_IP}"
echo "VM2 IP: ${VM2_IP}"
echo "VM3 IP: ${VM3_IP}"
...
prepare_deployment() {
    ...
    cp vm3-docker-compose.yml ~/cockroachdb-secure/docker-compose.yml
...
deploy_services() {
    print_status "Deploying VM3 services..."
    ...
    if docker exec roach-cert-vm3 ls -la /certs/ca.crt >/dev/null 2>&1; then
    ...
    if [ -f "production.env" ]; then
        docker-compose --env-file production.env up -d roach-2
    else
        docker-compose up -d roach-2
    fi
    ...
    while [ $retries -lt $max_retries ]; do
        if docker exec roach-2-vm3 ./cockroach node status --certs-dir=/certs --host=localhost:${COCKROACH_PORT} >/dev/null 2>&1; then
...
show_connection_info() {
    echo ""
    echo "=== VM3 Connection Information ==="
    echo "DB Console: https://${VM3_IP}:${CONSOLE_PORT}"
    echo "CockroachDB Port: ${VM3_IP}:${COCKROACH_PORT}"
    echo ""
    echo "=== Management Commands ==="
    echo "Check node status:"
    echo "  docker exec -it roach-2-vm3 ./cockroach node status --certs-dir=/certs --host=localhost:${COCKROACH_PORT}"
    echo ""
    echo "Access SQL shell:"
    echo "  docker exec -it roach-2-vm3 ./cockroach sql --certs-dir=/certs --host=localhost:${COCKROACH_PORT}"
    echo ""
    echo "View logs:"
    echo "  docker-compose logs -f roach-2"
    echo ""
...
show_help() {
    echo "CockroachDB VM3 Deployment Script"
    echo ""
    ...
    echo ""
    echo "Note: Deploy VM1 (primary node) first before deploying VM3"
}

# Function to stop services
stop_services() {
    print_status "Stopping VM3 services..."
    cd ~/cockroachdb-secure
    ...
    print_success "VM3 services stopped"
}

# Function to restart services
restart_services() {
    print_status "Restarting VM3 services..."
    cd ~/cockroachdb-secure
    ...
    print_success "VM3 services restarted"
}
...
main() {
    ...
    echo "========================================"
    echo "CockroachDB VM3 (Secondary Node) Deployment"
    echo "========================================"
    echo ""
    ...
    case $mode in
        "deploy")
            ...
            read -p "Deploy VM3 (Secondary Node)? (y/n): " -n 1 -r
            echo
            if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    ...
    print_success "VM3 deployment script completed!"
}
